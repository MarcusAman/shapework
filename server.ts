...
import { calculateAssessmentScore } from './src/services/scoringEngine';
...
const seedAssessmentResponses = [
  {
    ...
    numberOfAgents: '51-100',
    numberOfOfficeStaff: 4,
    numberOfLocations: 2,
    primaryMarket: 'Wilmington, NC',
    status: 'completed',
    answers: {
      pulledIntoIssues: 4,
      ...
      noOperatingRecord: 3,
      ownershipClarity: 3,
      recurringIssueOwner: 'Mostly',
      escalationFrequency: 'Weekly',
      interruptionsParagraph: 'Agents repeatedly calling to check if their commission checks are signed and if signs have been ordered.',
      singlePersonDependence: 'Accounting is fully dependent on Sarah.',
      unavailabilityBreak: 'Commission payouts and agent onboarding would freeze completely.',
      frictionAreas: ['Commission Processing', 'Agent Onboarding', 'Compliance', 'Property Signs'],
      greatestCostArea: 'Commission Processing',
      greatestFrustrationArea: 'Compliance',
      payToSolveFirstArea: 'Commission Processing',
      frictionFrequency: 'Weekly',
      currentWorkaround: 'Manual verification in Excel and sending slack messages.',
      systemsUsed: ['Dotloop', 'Google Workspace', 'Slack', 'QuickBooks'],
      frustratingSystem: 'Dotloop has a very high learning curve for new agents.',
      underutilizedSystem: 'QuickBooks is only used by accounting for basic entry.',
      duplicateDataFlows: 'Re-entering deal numbers and commission rates from Dotloop into Excel, then manually into QuickBooks.',
      spreadsheetDependencies: 'Commission payouts sheet, active listing sign tracker.',
      systemsConfidence: 3,
      leadershipHoursLost: '11-20',
      leadershipInterrupts: 'Urgent sign questions and commission check approvals.',
      leadershipApprovals: 'Every check payout requires physical signing.',
      leadershipHandsOff: 'Approving signage coordinates.',
      aiImplemented: 'Planning',
      aiTimeSavingWorkflow: 'Checking deal compliance documents automatically.',
      aiDreamWorkflow: 'A Slack bot that answers simple agent policy questions.',
      aiConcerns: ['Data privacy', 'Accuracy'],
      aiAdoptionReady: 4,
      scaleBreakPoints: 'Manual compliance review of deal folders.',
      growthLimitWorkflow: 'Compliance checking.',
      growthCostMoney: 'Manual labor chasing missing documents.',
      agentFrustrations: 'Delayed payouts.',
      leadershipFrustrations: 'Chasing agents for signatures.',
      locationScaleEase: 2,
      oneSolveThisYear: 'Automated Commission Routing and Sign ordering.',
      pilotInterest: 'Yes',
      reportConsent: 'Yes',
      followUpConsent: 'Yes'
    },
    scores: {
      ...
      categoryScores: {
        ownershipClarity: 60,
        leadershipLoad: 45,
        workflowConsistency: 48,
        technologyReadiness: 55,
        aiReadiness: 50,
        scalability: 40,
        operationalVisibility: 52
      },
      topRisks: [
        'Growth Scalability (Score: 40/100)',
        'Leadership Load (Score: 45/100)',
        'Workflow Consistency (Score: 48/100)'
      ],
      topOpportunities: [
        'Leadership Deflection and Task Delegator',
        'Departmental Escalation Routing System',
        'Background AI Agents for File Compliance Checklists'
      ],
      recommendedCategory: 'Growth Scalability'
    },
    internalClassification: {
      primaryPainCategory: 'Commission Processing',
      secondaryPainCategory: 'Compliance',
      frequency: 'Daily',
      financialImpact: '$15,000/year in labor leakage',
      urgency: 'High',
      repeatability: 'High',
      willingnessToPay: 'Medium',
      implementationComplexity: 'Medium',
      existingToolCoverage: 'Partial',
      productOpportunityScore: 78,
      notes: 'Strong candidate for pilot. High interest in commission flow automation.',
  ...
  {
    id: 'as_2',
    brokerageName: 'Realty One Group NC',
    respondentName: 'Marc Miller',
    emailAddress: 'marc@realtyone.nc',
    role: 'Owner',
    numberOfAgents: '101-250',
    numberOfOfficeStaff: 6,
    numberOfLocations: 4,
    primaryMarket: 'Raleigh/Durham',
    status: 'completed',
    answers: {
      pulledIntoIssues: 3,
      workFallsThroughCracks: 2,
      infoInSilos: 3,
      processesChange: 3,
      waitingOnApprovals: 4,
      repeatedQuestions: 4,
      struggleFindInfo: 3,
      sideConversations: 4,
      bottleneckPerson: 3,
      ...
      interruptionsParagraph: 'Compliance check delays and sign installation tracking issues.',
      singlePersonDependence: 'Office staff handling onboarding.',
      unavailabilityBreak: 'New agent onboarding would stall.',
      frictionAreas: ['Agent Onboarding', 'Listing Management', 'Compliance'],
      greatestCostArea: 'Agent Onboarding',
      greatestFrustrationArea: 'Compliance',
      payToSolveFirstArea: 'Compliance',
      frictionFrequency: 'Weekly',
      currentWorkaround: 'Shared email inboxes.',
      systemsUsed: ['SkySlope', 'Follow Up Boss', 'Slack', 'Asana'],
      frustratingSystem: 'SkySlope doc upload UI.',
      underutilizedSystem: 'Asana templates.',
      duplicateDataFlows: 'Syncing lead details between Follow Up Boss and SkySlope.',
      spreadsheetDependencies: 'Agent roster spreadsheet.',
      systemsConfidence: 4,
      leadershipHoursLost: '6-10',
      leadershipInterrupts: 'Compliance queries.',
      leadershipApprovals: 'Listing launch authorization.',
      leadershipHandsOff: 'Roster management.',
      aiImplemented: 'Yes, in a few areas',
      aiTimeSavingWorkflow: 'Writing listing descriptions.',
      aiDreamWorkflow: 'Automated compliance document sorting.',
      aiConcerns: ['Data privacy', 'Accuracy'],
      aiAdoptionReady: 3,
      scaleBreakPoints: 'Roster scaling.',
      growthLimitWorkflow: 'Onboarding.',
      growthCostMoney: 'Delayed listing launches.',
      agentFrustrations: 'Chasing checklist items.',
      leadershipFrustrations: 'Agent delay in uploading docs.',
      locationScaleEase: 3,
      oneSolveThisYear: 'Automated SkySlope Checklist chaser.',
      pilotInterest: 'Maybe',
      reportConsent: 'Yes',
      followUpConsent: 'Yes'
    },
    scores: {
      ...
      categoryScores: {
        ownershipClarity: 72,
        leadershipLoad: 65,
        workflowConsistency: 62,
        technologyReadiness: 70,
        aiReadiness: 70,
        scalability: 64,
        operationalVisibility: 71
      },
      topRisks: [
        'Workflow Consistency (Score: 62/100)',
        'Growth Scalability (Score: 64/100)',
        'Leadership Load (Score: 65/100)'
      ],
      topOpportunities: [
        'Leadership Deflection and Task Delegator',
        'Departmental Escalation Routing System',
      ...
      recommendedCategory: 'Workflow Consistency'
    },
    internalClassification: {
      primaryPainCategory: 'Compliance',
      secondaryPainCategory: 'Agent Onboarding',
      frequency: 'Weekly',
      financialImpact: '$8,000/year',
      urgency: 'Medium',
      repeatability: 'High',
      willingnessToPay: 'Medium',
      implementationComplexity: 'Low',
      existingToolCoverage: 'None',
      productOpportunityScore: 65,
      notes: 'Interested in AI compliance checks. Follow up in Q3.',
      status: 'review_completed',
      assignedOwner: 'usr_sarah'
    },
...
const defaultDbState = {
  ...
  assessmentResponses: [...seedAssessmentResponses],
...
export const cameraCredentialsVault = new Map<string, { username?: string; password?: string }>();
const lastSnapshotTimestamps = new Map<string, number>();

const MOCK_IMAGE_BUFFER = Buffer.from(
  '',
  'base64'
);
...
function testTcpPort(host: string, port: number, timeoutMs = 3000): Promise<boolean> {
  return new Promise((resolve) => {
...
function testFfprobeStream(rtspUrl: string, timeoutMs = 3000): Promise<{ valid: boolean; error?: string }> {
  return new Promise((resolve) => {
    const cmd = `ffprobe -v error -rtsp_transport tcp -show_entries format=format_name -of default=noprint_wrappers=1 "${rtspUrl}"`;
    
    exec(cmd, { timeout: timeoutMs }, (error, stdout, stderr) => {
      if (error) {
        const rawErr = error.message || stderr || String(error);
        const maskedErr = rawErr.replace(/:[^@:]+@/g, ':********@');
        resolve({ valid: false, error: maskedErr });
      } else {
        resolve({ valid: true });
      }
    });
...
function captureSnapshotWithFfmpeg(rtspUrl: string, outputPath: string, timeoutMs = 10000): Promise<{ success: boolean; error?: string }> {
  return new Promise((resolve) => {
    const cmd = `ffmpeg -y -rtsp_transport tcp -i "${rtspUrl}" -frames:v 1 -q:v 2 "${outputPath}"`;
    
    exec(cmd, { timeout: timeoutMs }, (error, stdout, stderr) => {
      if (error) {
        const rawErr = error.message || stderr || String(error);
        const maskedErr = rawErr.replace(/:[^@:]+@/g, ':********@');
        resolve({ success: false, error: maskedErr });
      } else {
...
function initializeCameraCredentials() {
  const mainUsername = process.env.TAPO_CAMERA_USERNAME || 'MarcusAman';
  const mainPassword = process.env.TAPO_CAMERA_PASSWORD || 'M@rk1683!!';
  
  cameraCredentialsVault.set('cam_sign_room_001', {
    username: mainUsername,
    password: mainPassword
  });
  
  if (dbState && dbState.opsCameras) {
    dbState.opsCameras.forEach((cam: any) => {
      if (cam.id === 'cam_sign_room_001') {
        if (mainUsername || mainPassword) {
          cameraCredentialsVault.set(cam.id, {
            username: mainUsername,
            password: mainPassword
          });
        }
...
// GET /api/internal/cameras - Configured cameras list (sanitized metadata)
app.get('/api/internal/cameras', requireAuth, requireInternal, (req, res) => {
  ...
  const sanitized = cameras.map((c: any) => ({
    ...c,
    passwordRef: c.passwordRef ? '********' : ''
  }));
  res.json({ success: true, cameras: sanitized });
});

// POST /api/internal/cameras - Add/Configure camera
app.post('/api/internal/cameras', requireAuth, requireInternal, (req, res) => {
  const workspaceId = req.headers['x-workspace-id'] as string || 'nest-realty-demo';
  const { id, provider, name, locationLabel, cameraIp, rtspPort, onvifPort, username, password, streamPathHigh, streamPathLow } = req.body;
  
  if (!name || !cameraIp) {
    return res.status(400).json({ error: 'Bad Request', message: 'Camera name and IP are required.' });
  }
  
  const cameraId = id || `cam_${Date.now()}`;
  let camera = (dbState.opsCameras || []).find((c: any) => c.id === cameraId);
  const isNew = !camera;
  ...
  if (isNew) {
    ...
    if (!dbState.opsCameras) dbState.opsCameras = [];
    dbState.opsCameras.push(camera);
  }
  
  camera.provider = provider || 'tapo';
  camera.name = name;
  camera.locationLabel = locationLabel || '';
  camera.locationName = locationLabel || '';
  camera.cameraIp = cameraIp;
  camera.rtspPort = Number(rtspPort) || 554;
  camera.onvifPort = Number(onvifPort) || 2020;
  camera.usernameRef = username || 'admin';
  camera.passwordRef = password ? '********' : (camera.passwordRef || '');
  camera.streamPathHigh = streamPathHigh || '/stream1';
  camera.streamPathLow = streamPathLow || '/stream2';
  camera.status = camera.status || 'unknown';
  camera.updatedAt = new Date().toISOString();
  
  if (username || password) {
    const existing = cameraCredentialsVault.get(cameraId) || {};
    cameraCredentialsVault.set(cameraId, {
  ...
  persistState();
  res.json({ success: true, camera: { ...camera, passwordRef: camera.passwordRef ? '********' : '' } });
});

// POST /api/internal/cameras/test - Connection test
app.post('/api/internal/cameras/test', requireAuth, requireInternal, async (req, res) => {
  const { cameraId, host, rtspPort, onvifPort, streamPath } = req.body;
  
  let targetHost = host;
  let targetRtspPort = Number(rtspPort) || 554;
  let targetOnvifPort = Number(onvifPort) || 2020;
  let targetStreamPath = streamPath || '/stream1';
  let username = '';
  let password = '';
  
  if (cameraId) {
    const camera = (dbState.opsCameras || []).find((c: any) => c.id === cameraId);
    if (camera) {
      targetHost = targetHost || camera.cameraIp;
      targetRtspPort = targetRtspPort || camera.rtspPort;
      targetOnvifPort = targetOnvifPort || camera.onvifPort;
      targetStreamPath = targetStreamPath || camera.streamPathHigh;
      
      const creds = cameraCredentialsVault.get(cameraId);
      if (creds) {
  ...
  if (!username) username = process.env.TAPO_CAMERA_USERNAME || 'admin';
  if (!password) password = process.env.TAPO_CAMERA_PASSWORD || '';
  if (!targetHost) targetHost = process.env.TAPO_CAMERA_HOST || '127.0.0.1';
  
  if (!targetHost) {
    return res.status(400).json({ error: 'Bad Request', message: 'Camera host/IP is required.' });
  }
  
  console.log(`[Camera Diagnostics] Connection test on ${targetHost}`);
  const rtspReachable = await testTcpPort(targetHost, targetRtspPort, 3000);
  const onvifReachable = await testTcpPort(targetHost, targetOnvifPort, 3000);
  
  let ffprobeAvailable = false;
  let ffprobeResult: { valid: boolean; error?: string } = { valid: false, error: 'ffprobe is not available' };
  
  try {
    ffprobeAvailable = await isCommandAvailable('ffprobe');
    if (ffprobeAvailable && rtspReachable) {
      const encodedUser = encodeURIComponent(username);
      const encodedPass = encodeURIComponent(password);
      const rtspUrl = `rtsp://${encodedUser}:${encodedPass}@${targetHost}:${targetRtspPort}${targetStreamPath}`;
      ffprobeResult = await testFfprobeStream(rtspUrl, 3000);
    }
  } catch (err: any) {
    ffprobeResult = { valid: false, error: err.message };
  }
  
  let status: 'online' | 'offline' | 'error' = 'offline';
  if (rtspReachable && (!ffprobeAvailable || ffprobeResult.valid)) {
    status = 'online';
  } else if (rtspReachable || onvifReachable) {
    status = 'error';
  }
  ...
  res.json({
    ...
    diagnostics: {
      host: targetHost,
      rtspPort: targetRtspPort,
      onvifPort: targetOnvifPort,
      rtspReachable,
      onvifReachable,
      ffprobeAvailable,
      ffprobeValid: ffprobeResult.valid,
...
// POST /api/internal/cameras/snapshot - Capture snapshot
app.post('/api/internal/cameras/snapshot', requireAuth, requireInternal, async (req, res) => {
  const { cameraId, reason, eventType } = req.body;
  
  if (!cameraId) {
    return res.status(400).json({ error: 'Bad Request', message: 'cameraId is required.' });
  }
  ...
  const now = Date.now();
  const lastTime = lastSnapshotTimestamps.get(cameraId) || 0;
  if (now - lastTime < 5000) {
    return res.status(429).json({
      error: 'Too Many Requests',
      message: 'Snapshot capture is rate-limited to once every 5 seconds.'
    });
  }
  lastSnapshotTimestamps.set(cameraId, now);
  ...
  const host = camera.cameraIp || process.env.TAPO_CAMERA_HOST || '127.0.0.1';
  const rtspPort = camera.rtspPort || 554;
  const streamHigh = camera.streamPathHigh || '/stream1';
  const streamLow = camera.streamPathLow || '/stream2';
  
  const dateFolder = getFormattedDateFolder();
  const timestampStr = getFormattedTimestamp();
  const typeStr = eventType || 'manual_snapshot';
  const filename = `${cameraId}_${timestampStr}_${typeStr}.jpg`;
  
  const uploadsDir = path.join(process.cwd(), 'uploads', 'camera-events', dateFolder);
  ...
  if (ffmpegAvailable) {
    const encodedUser = encodeURIComponent(username);
    const encodedPass = encodeURIComponent(password);
    const rtspUrlHigh = `rtsp://${encodedUser}:${encodedPass}@${host}:${rtspPort}${streamHigh}`;
    let captureRes = await captureSnapshotWithFfmpeg(rtspUrlHigh, outputPath, 10000);
    
    if (captureRes.success) {
      success = true;
    } else {
      console.warn(`[Camera Snapshot] High quality failed, retrying low quality...`);
      const rtspUrlLow = `rtsp://${encodedUser}:${encodedPass}@${host}:${rtspPort}${streamLow}`;
      captureRes = await captureSnapshotWithFfmpeg(rtspUrlLow, outputPath, 10000);
      if (captureRes.success) {
        success = true;
      } else {
        errorMsg = captureRes.error || 'Capture failed on both streams.';
      }
    }
  } else {
    errorMsg = 'ffmpeg is required for snapshot capture.';
  }
  
  if (!success) {
    try {
      fs.writeFileSync(outputPath, MOCK_IMAGE_BUFFER);
    } catch (e) {}
  }
  ...
  const newEvent = {
    ...
    locationLabel: camera.locationLabel || camera.locationName || '',
    eventType: typeStr,
  ...
  dbState.opsAuditLogs.unshift({
    ...
    actorName: 'Ann',
    action: success ? 'camera_snapshot_captured' : 'camera_snapshot_failed',
  ...
  camera.lastSnapshotAt = new Date().toISOString();
...
// GET /api/internal/cameras/events - Camera events list
app.get('/api/internal/cameras/events', requireAuth, requireInternal, (req, res) => {
  const workspaceId = req.headers['x-workspace-id'] as string || 'nest-realty-demo';
  const events = (dbState.opsCameraEvents || []).filter((e: any) => e.workspaceId === workspaceId);
  res.json({ success: true, events });
});

// GET /api/internal/cameras/events/:eventId/image - Image serve
app.get('/api/internal/cameras/events/:eventId/image', requireAuth, requireInternal, (req, res) => {
  const { eventId } = req.params;
  const event = (dbState.opsCameraEvents || []).find((e: any) => e.id === eventId);
  if (!event) {
    return res.status(404).json({ error: 'Not Found', message: 'Event not found.' });
  }
  
  const imagePath = event.snapshotPath;
  if (!imagePath || !fs.existsSync(imagePath)) {
    if (event.snapshotUrl) {
      return res.redirect(event.snapshotUrl);
    }
    res.setHeader('Content-Type', 'image/png');
    return res.send(MOCK_IMAGE_BUFFER);
  }
...
// POST /api/internal/cameras/events - External manual ingest
app.post('/api/internal/cameras/events', requireAuth, requireInternal, (req, res) => {
  const { eventType, cameraId, reason, status } = req.body;
  const workspaceId = req.headers['x-workspace-id'] as string || 'nest-realty-demo';
  
  if (!cameraId) {
    return res.status(400).json({ error: 'Bad Request', message: 'Missing cameraId.' });
  }
  ...
  const newEvent = {
    ...
    reason: reason || 'External API event ingestion',
    snapshotPath: '',
...
// --- ORG KNOWLEDGE BASE ENDPOINTS ---
const KNOWLEDGE_DB_PATH = path.join(process.cwd(), 'data', 'org-knowledge.json');
const getKnowledgeDocs = (): any[] => {
...
const saveKnowledgeDocs = (docs: any[]) => {
  const dir = path.dirname(KNOWLEDGE_DB_PATH);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  fs.writeFileSync(KNOWLEDGE_DB_PATH, JSON.stringify(docs, null, 2), 'utf-8');
};
...
app.post('/api/org-knowledge/upload', (req, res) => {
  try {
    const { 
      workspaceId, fileName, fileContent, ownerPositionId, ownerRoleId, 
      requestCategories, tags, documentType, includeInAskNestOps, includeInRetell, includeInRouting 
    } = req.body;

    if (!workspaceId || !fileName || !fileContent) {
      return res.status(400).json({ error: 'Missing required parameters: workspaceId, fileName, fileContent' });
    }

    // 1. Sanitize filename to prevent directory traversal
    const safeFileName = path.basename(fileName).replace(/[^a-zA-Z0-9.-]/g, '_');
    const uploadDir = path.join(process.cwd(), 'uploads', 'org-knowledge', workspaceId);
    ...
    // 2. Decode base64 and write file
    ...
    // 3. Document text extraction for TXT/MD
    const ext = path.extname(safeFileName).toLowerCase();
    let extractedText = '';
    let aiSummary = '';
    let extractionStatus: 'uploaded' | 'pending_extraction' | 'extracting' | 'extracted' | 'needs_review' | 'failed' = 'pending_extraction';

    if (ext === '.txt' || ext === '.md') {
      extractedText = fileBuffer.toString('utf-8');
      extractionStatus = 'extracted';
      aiSummary = `Extracted text document: ${extractedText.substring(0, 120)}...`;
    } else {
      aiSummary = `Document uploaded. Text extraction and AI summary will be available after backend processing is enabled.`;
    }
    ...
    const newDoc = {
      id: `kd_${Date.now()}`,
      workspaceId,
      title: safeFileName.replace(/\.[^/.]+$/, "").replace(/_/g, ' '),
      sourceType: ext.slice(1) || 'txt',
      ...
      aiSummary: aiSummary,
      extractionStatus,
      includeInAskNestOps: includeInAskNestOps ?? true,
      includeInRetell: includeInRetell ?? true,
      includeInRouting: includeInRouting ?? true,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };
...
app.get('/api/org-knowledge', (req, res) => {
  ...
  const docs = getKnowledgeDocs();
  if (workspaceId) {
    res.json(docs.filter(d => d.workspaceId === workspaceId));
  } else {
    res.json(docs);
  }
});

// GET /api/org-knowledge/:id
app.get('/api/org-knowledge/:id', (req, res) => {
  const { id } = req.params;
  const docs = getKnowledgeDocs();
...
// PUT /api/org-knowledge/:id
app.put('/api/org-knowledge/:id', (req, res) => {
  const { id } = req.params;
...
// POST /api/org-knowledge/:id/extract
app.post('/api/org-knowledge/:id/extract', (req, res) => {
  const { id } = req.params;
  const docs = getKnowledgeDocs();
  const index = docs.findIndex(d => d.id === id);
  if (index === -1) {
  ...
  doc.aiSummary = `Successfully extracted content from file ${doc.fileName}. Summarized operational guidelines are fully parsed and searchable.`;
  doc.suggestedSteps = ['Review closing package parameters', 'Cross-check escrow ledger values', 'Verify BIC signoff'];
  doc.suggestedTriggers = [`Whenever new ${doc.title} task is generated`];
  doc.suggestedRequiredInformation = ['Property Address', 'Responsible Agent', 'Escrow Agent Contact'];
...
app.get('/api/org-knowledge/:id/download', (req, res) => {
  const { id } = req.params;
  const docs = getKnowledgeDocs();
  const doc = docs.find(d => d.id === id);
...
// Helper functions for assessment DB abstraction
async function saveResponseToDb(resData: any) {
  if (dbPool) {
    ...
    await dbPool.query(q, [
      resData.brokerageName,
      resData.respondentName,
      resData.emailAddress,
      resData.role,
      resData.numberOfAgents,
      Number(resData.numberOfOfficeStaff) || 0,
      Number(resData.numberOfLocations) || 0,
      resData.primaryMarket || '',
      resData.status || 'in_progress',
      JSON.stringify(resData.answers || {}),
      JSON.stringify(resData.scores || {}),
      JSON.stringify(resData.internalClassification || {}),
  ...
  if (idx > -1) {
    ...
    dbState.assessmentResponses.unshift(resData);
  }
  persistState();
}
...
// 1. POST /api/public/assessments - Create new response from public form
app.post('/api/public/assessments', async (req, res) => {
  try {
    const answers = req.body;
    if (!answers.brokerageName || !answers.respondentName || !answers.emailAddress || !answers.role || !answers.numberOfAgents) {
      return res.status(400).json({ error: 'Bad Request', message: 'Missing required profile fields.' });
    }

    const calculatedScores = calculateAssessmentScore(answers);
    const newResponse = {
      id: `as_${Date.now()}`,
      brokerageName: answers.brokerageName,
      respondentName: answers.respondentName,
      emailAddress: answers.emailAddress,
      role: answers.role,
      numberOfAgents: answers.numberOfAgents,
      numberOfOfficeStaff: Number(answers.numberOfOfficeStaff) || 0,
      numberOfLocations: Number(answers.numberOfLocations) || 0,
      primaryMarket: answers.primaryMarket || '',
      status: 'completed',
      answers,
      scores: calculatedScores,
      internalClassification: {
        primaryPainCategory: answers.greatestCostArea || 'Compliance',
        secondaryPainCategory: answers.greatestFrustrationArea || 'Onboarding',
        frequency: answers.frictionFrequency || 'Weekly',
        financialImpact: 'TBD',
        urgency: 'Medium',
        repeatability: 'High',
        willingnessToPay: 'Medium',
    ...
    await saveResponseToDb(newResponse);
    res.json({ success: true, id: newResponse.id, scores: calculatedScores });
  } catch (err: any) {
    console.error('Failed to create assessment response:', err);
    res.status(500).json({ error: 'Server Error', message: err.message });
  }
});

// 2. GET /api/public/assessments/:id - Load in-progress/completed response (public access check)
app.get('/api/public/assessments/:id', async (req, res) => {
  try {
    ...
    // Return safe public fields only (hide internalClassification)
    res.json({
      success: true,
      response: {
        id: response.id,
        ...
        role: response.role,
        numberOfAgents: response.numberOfAgents,
        numberOfOfficeStaff: response.numberOfOfficeStaff,
...
// 3. PUT /api/public/assessments/:id - Update in-progress/completed response
app.put('/api/public/assessments/:id', async (req, res) => {
  try {
    const answers = req.body;
    const response = await getResponseByIdFromDb(req.params.id);
    if (!response) {
    ...
    const updatedResponse = {
      ...response,
      brokerageName: answers.brokerageName || response.brokerageName,
      respondentName: answers.respondentName || response.respondentName,
      emailAddress: answers.emailAddress || response.emailAddress,
      role: answers.role || response.role,
      numberOfAgents: answers.numberOfAgents || response.numberOfAgents,
      numberOfOfficeStaff: Number(answers.numberOfOfficeStaff) || response.numberOfOfficeStaff,
      numberOfLocations: Number(answers.numberOfLocations) || response.numberOfLocations,
      primaryMarket: answers.primaryMarket || response.primaryMarket,
      answers,
      scores: calculatedScores,
      updatedAt: new Date().toISOString()
    };

    await saveResponseToDb(updatedResponse);
    res.json({ success: true, scores: calculatedScores });
  } catch (err: any) {
    res.status(500).json({ error: 'Server Error', message: err.message });
  }
});

// 4. GET /api/assessments - Private list (requires internal auth)
...
// 5. GET /api/assessments/:id - Private detail (requires internal auth)
...
// 6. POST /api/assessments/:id/classify - Private classification update
app.post('/api/assessments/:id/classify', requireAuth, requireInternal, async (req, res) => {
  try {
    const response = await getResponseByIdFromDb(req.params.id);
    if (!response) {
      return res.status(404).json({ error: 'Not Found', message: 'Response not found.' });
    ...
    const { primaryPainCategory, secondaryPainCategory, frequency, financialImpact, urgency, repeatability, willingnessToPay, implementationComplexity, existingToolCoverage, productOpportunityScore, notes, status, assignedOwner } = req.body;
    
    const updatedResponse = {
      ...response,
      internalClassification: {
        primaryPainCategory: primaryPainCategory || response.internalClassification?.primaryPainCategory || '',
        secondaryPainCategory: secondaryPainCategory || response.internalClassification?.secondaryPainCategory || '',
        frequency: frequency || response.internalClassification?.frequency || '',
        financialImpact: financialImpact || response.internalClassification?.financialImpact || '',
        urgency: urgency || response.internalClassification?.urgency || '',
        repeatability: repeatability || response.internalClassification?.repeatability || '',
        willingnessToPay: willingnessToPay || response.internalClassification?.willingnessToPay || '',
        implementationComplexity: implementationComplexity || response.internalClassification?.implementationComplexity || '',
        existingToolCoverage: existingToolCoverage || response.internalClassification?.existingToolCoverage || '',
        productOpportunityScore: Number(productOpportunityScore) || response.internalClassification?.productOpportunityScore || 50,
        notes: notes || response.internalClassification?.notes || '',
        status: status || response.internalClassification?.status || 'new',
        assignedOwner: assignedOwner || response.internalClassification?.assignedOwner || 'usr_sarah'
      },
      updatedAt: new Date().toISOString()
    };
...
// 7. GET /api/market-intelligence - Private aggregate analytics
...
// 8. GET /api/assessments/:id/export/csv - Export individual response as CSV
app.get('/api/assessments/:id/export/csv', requireAuth, requireInternal, async (req, res) => {
  try {
    const response = await getResponseByIdFromDb(req.params.id);
    if (!response) {
    ...
    let csv = 'Brokerage Operational Intelligence Assessment - Individual Response\n';
    csv += `ID,${response.id}\n`;
    csv += `Brokerage,${response.brokerageName.replace(/"/g, '""')}\n`;
    csv += `Respondent,${response.respondentName.replace(/"/g, '""')}\n`;
    csv += `Email,${response.emailAddress}\n`;
    csv += `Role,${response.role}\n`;
    csv += `Agents,${response.numberOfAgents}\n`;
    csv += `Office Staff,${response.numberOfOfficeStaff}\n`;
    csv += `Locations,${response.numberOfLocations}\n`;
    csv += `Region,${response.primaryMarket?.replace(/"/g, '""') || ''}\n`;
    csv += `Status,${response.status}\n`;
    csv += `Overall Score,${response.scores?.overallScore || 'N/A'}\n\n`;

    csv += 'Category,Score\n';
    if (response.scores?.categoryScores) {
      Object.entries(response.scores.categoryScores).forEach(([cat, val]) => {
        csv += `${cat},${val}\n`;
      });
    }
    ...
    csv += '\nQuestion,Answer\n';
    if (response.answers) {
      Object.entries(response.answers).forEach(([q, val]) => {
        let valStr = '';
        if (Array.isArray(val)) valStr = val.join('; ');
        else if (typeof val === 'object') valStr = JSON.stringify(val);
        else valStr = String(val);
        csv += `"${q}","${valStr.replace(/"/g, '""')}"\n`;
      });
    }

    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename=assessment_${response.id}.csv`);
...
// 9. GET /api/assessments/export/csv - Export aggregate responses as CSV
app.get('/api/assessments/export/csv', requireAuth, requireInternal, async (req, res) => {
  try {
    const list = await getResponsesFromDb();
    let csv = 'ID,Brokerage,Respondent,Email,Role,Agents,Office Staff,Locations,Market,Status,Overall Score,Top Opportunities,Created Date\n';
    list.forEach((r: any) => {
      const opps = r.scores?.topOpportunities ? r.scores.topOpportunities.join('; ') : '';
      csv += `${r.id},"${r.brokerageName.replace(/"/g, '""')}",`;
      csv += `"${r.respondentName.replace(/"/g, '""')}",${r.emailAddress},`;
      csv += `"${r.role}",${r.numberOfAgents},${r.numberOfOfficeStaff || 0},`;
      csv += `${r.numberOfLocations || 0},"${r.primaryMarket?.replace(/"/g, '""') || ''}",`;
      csv += `${r.status},${r.scores?.overallScore || 0},"${opps.replace(/"/g, '""')}",${r.createdAt}\n`;
    });

    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', 'attachment; filename=assessments_aggregate.csv');
    res.status(200).send(csv);
  } catch (err: any) {
    res.status(500).send(err.message);
  }
