...
export const SEEDED_CAMERAS = [
  {
    ...
    provider: 'tapo' as const,
    locationLabel: 'Nest Realty Wilmington sign storage',
    locationName: 'Nest Realty Wilmington sign storage',
    cameraIp: process.env.TAPO_CAMERA_HOST || '192.168.68.56',
    rtspPort: Number(process.env.TAPO_CAMERA_RTSP_PORT) || 554,
    onvifPort: Number(process.env.TAPO_CAMERA_ONVIF_PORT) || 2020,
    usernameRef: process.env.TAPO_CAMERA_USERNAME || 'MarcusAman',
    passwordRef: process.env.TAPO_CAMERA_PASSWORD ? '********' : '********',
    streamPathHigh: process.env.TAPO_CAMERA_STREAM_HIGH || '/stream1',
    streamPathLow: process.env.TAPO_CAMERA_STREAM_LOW || '/stream2',
