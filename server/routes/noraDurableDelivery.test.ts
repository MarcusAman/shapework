...
const state = vi.hoisted(() => ({ tasks: new Map<string, any>(), events: [] as any[], send: vi.fn(), getAsset: vi.fn(), createToken: vi.fn(), resend: vi.fn(), approveQuery: vi.fn() }));
...
vi.mock('../services/marketingDeliveryLock.js', () => ({ acquireMarketingDeliveryLock: async () => async () => {}, refreshMarketingDeliveryState: async () => true, persistMarketingDeliverySnapshot: async (task: any, patch: any) => { task.routingSnapshot = {...task.routingSnapshot, ...patch}; }, beginMarketingDeliveryAttempt: async (task: any, key: string) => { if (task.routingSnapshot?.deliveryAttempt?.key === key && task.routingSnapshot.deliveryAttempt.status === 'sending') return false; task.routingSnapshot.deliveryAttempt = {key,status:'sending'}; return true; }, finishMarketingDeliveryAttempt: async (task: any, key: string, status: string) => { task.routingSnapshot.deliveryAttempt = {key,status}; } }));
...
describe('Nora durable delivery route lifecycle', () => {
  beforeEach(() => {
    ...
    state.tasks.clear(); state.events.length = 0; (globalThis as any).__NEST_OUTREACH_ONCE = new Map();
    ...
    state.approveQuery.mockImplementation(async (_sql: string, values: any[]) => {
      const current = state.tasks.get(values[0]);
      if (!current || current.workspaceId !== values[1] || current.proofVersion !== values[9]) return { rows: [] };
      return { rows: [{ routing_snapshot: current.routingSnapshot }] };
    });
  ...
  it('allows information request then delivery then a revised proof delivery', async () => {
    ...
    expect((await post()).status).toBe(200);
    task.proofVersion = 2; task.proofUrl = '/uploads/proof-2.pdf'; task.reviewState = 'awaiting_review';
    expect((await post()).status).toBe(200);
