...
export const ProofUploadWizard: React.FC<ProofUploadWizardProps> = ({
  ...
  // Wizard steps: 1: Select deliverable, 2: Choose/Drop files, 3: Inspect, 4: Preview/Confirm
  const [step, setStep] = useState<1 | 2 | 3 | 4>(1);
  const [selectedDeliverable, setSelectedDeliverable] = useState<string>(() => {
    return requestedDeliverables[0]?.name || '8.5x11 Property Flyer';
  });
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden flex flex-col z-10 animate-in zoom-in-95 duration-150">
        ...
        {!storageConfigured && (
          <div className="mx-6 mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-2.5 text-xs text-amber-900">
            ...
            <div>
              <span className="font-bold">Cloud Object Storage Notice: </span>
              <span>Durable cloud bucket is not configured in this local environment. File inspection, metadata verification, and browser preview will run locally. Manual Google Drive / Maxa proof links remain fully functional.</span>
            </div>
          </div>
        )}
          ...
          {step === 1 && (
            <div className="space-y-4 animate-in fade-in duration-150">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  {
                    id: '8.5x11 Property Flyer',
                    label: '8.5 × 11 Flyer',
                    spec: 'Print Letter / PDF',
                    icon: <FileText className="w-4 h-4 text-[#00635C]" />
                  },
                  {
                    id: '9:16 Social Story Carousel',
                    label: '9:16 Social Story',
                    spec: '1080 × 1920 px (Carousel)',
                    icon: <ImageIcon className="w-4 h-4 text-blue-600" />
                  },
                  {
                    id: '6x9 EDDM Direct Mail Postcard',
                    label: '6 × 9 Postcard',
                    spec: 'Direct Mail / EDDM',
                    icon: <Layers className="w-4 h-4 text-purple-600" />
                  }
                ].map((deliv) => {
                  const isSelected = selectedDeliverable === deliv.id;
                  return (
                    <button
                      key={deliv.id}
                      type="button"
                      onClick={() => setSelectedDeliverable(deliv.id)}
                    ...
                    >
                      ...
                      <div>
                        <div className="text-xs font-bold text-slate-900">{deliv.label}</div>
                        <div className="text-[11px] text-slate-500 mt-0.5">{deliv.spec}</div>
                      </div>
                    </button>
                  );
                })}
