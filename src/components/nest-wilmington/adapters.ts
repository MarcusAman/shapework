...
export function buildRoleEscalationMap(model: OrgModel): RoleEscalationData {
  ...
  const roleMap: RoleMapCard[] = positions.map(p => ({
    id: p.id,
    name: p.name,
    title: p.title,
    department: p.department || 'Brokerage',
    handles: p.roleIds && p.roleIds.length > 0 ? p.roleIds : ['Client Representation', 'Deal Execution', 'Compliance'],
