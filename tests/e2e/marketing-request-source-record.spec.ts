...
test.describe('Marketing Request Source Record', () => {
  test('Opens original communication drawer showing verbatim transcript and AI summary', async ({ page }) => {
    ...
    await page.locator('[data-testid="open-original-communication-btn"]').dispatchEvent('click');
    await page.waitForSelector('[data-testid="original-communication-drawer"]');
    ...
    await page.locator('[data-testid="close-communication-drawer"]').dispatchEvent('click');
