...
describe('Ask Nest Ops — Voice Pipeline & Idempotency Architecture', () => {
  ...
  it('10. Lorena wake phrase variants ("Hey Lorena", "Hi Lorena") trigger WAKE_WORD_ONLY response', () => {
    const mockState: any = { pendingProposal: null };
    const res1 = processUserUtterance('Hey Lorena', mockState, 'Ryan', 'utt_lorena_1');
    expect(res1.intentType).toBe('WAKE_WORD_ONLY');
    expect(res1.spokenResponse).toBe("Hi, I'm listening.");

    const res2 = processUserUtterance('Hi Lorena', mockState, 'Ryan', 'utt_lorena_2');
    expect(res2.intentType).toBe('WAKE_WORD_ONLY');
    expect(res2.spokenResponse).toBe("Hi, I'm listening.");
  });

  it('11. Lorena correctly routes Multimodal Vision Document Scan intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, scan this document camera image for Form 2-T', mockState, 'Ryan', 'utt_lorena_vision');
  ...
  it('12. Lorena correctly routes Buyer-Seller Matchmaker Radar intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, run buyer matchmaker radar for 312 Mayfaire', mockState, 'Ryan', 'utt_lorena_radar');
  ...
  it('13. Lorena correctly routes Deal Celebration & 3D Particle Universe intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, celebrate closed deal at 312 Mayfaire Way', mockState, 'Ryan', 'utt_lorena_celebration');
  ...
  it('14. Lorena correctly routes MLS Listing Launch intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, launch mls listing for 312 Mayfaire Way', mockState, 'Ryan', 'utt_lorena_mls');
  ...
  it('15. Lorena correctly routes Commission Split & Payroll Copilot intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, check commission split payroll for Sarah Jenkins', mockState, 'Ryan', 'utt_lorena_commission');
  ...
  it('16. Lorena correctly routes Seller Net Sheet Calculator intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, calculate seller net sheet for 312 Mayfaire Way', mockState, 'Ryan', 'utt_lorena_netsheet');
  ...
  it('17. Lorena correctly routes Comparative Market Analysis (CMA) intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, run cma market analysis for 312 Mayfaire Way', mockState, 'Ryan', 'utt_lorena_cma');
  ...
  it('18. Lorena correctly routes Open House Kiosk intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, launch open house kiosk for 312 Mayfaire Way', mockState, 'Ryan', 'utt_lorena_kiosk');
  ...
  it('19. Lorena correctly routes Commercial Lease Audit intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, audit commercial lease for Suite 400', mockState, 'Ryan', 'utt_lorena_comm');

    expect(result.intentType).toBe('AUDIT_COMMERCIAL_LEASE');
    expect(result.spokenResponse).toContain('Pinnacle Tech Solutions');
  });

  it('20. Lorena correctly routes Emergency Maintenance Dispatch intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, emergency plumber water heater maintenance at Unit B', mockState, 'Ryan', 'utt_lorena_maint');

    expect(result.intentType).toBe('DISPATCH_PROPERTY_MAINTENANCE');
    expect(result.spokenResponse).toContain('Wilmington Mechanical');
  });

  it('21. Lorena correctly routes Multiple Offer Comparison Matrix intent', () => {
    const mockState: any = { pendingProposal: null };
    const result = processUserUtterance('Hey Lorena, compare all offers for 312 Mayfaire Way', mockState, 'Ryan', 'utt_lorena_offers');
